  ...
  app.post("/api/ai-parse", async (req, res) => {
    ...
    try {
      ...
      const response = await fetch(OLLAMA_URL, {
        ...
        body: JSON.stringify({
          model: "llama3.1", 
          prompt: `Ты — продвинутый планировщик школьных задач. Твоя цель: разбить длинное сообщение пользователя на ОТДЕЛЬНЫЕ задачи и перефразировать их так, чтобы они были короткими и понятными.
            ...
            3. Если дата не указана, используй "Завтра".
            
            Список предметов: Алгебра, Геометрия, Информатика, Робототехника, Русский язык, Литература, Английский язык, История, Обществознание, Физика, Химия, Биология, География.
            
            Текст пользователя: "${prompt}"
            
            Верни СТРОГО массив JSON объектов. Пример:
            [
              {"subject": "Алгебра", "task": "Решить №45, 46", "deadline": "24.04.2026", "priority": "high"},
              {"subject": "Литература", "task": "Выучить стих", "deadline": "25.04.2026", "priority": "medium"}
            ]`,
          stream: false,
          options: {
      ...
      let tasksArray = JSON.parse(jsonMatch[0]);
